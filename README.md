...
## 7. 수익화 — 제휴(어필리에이트) 링크 넣는 법

결과 카드의 **예스24 · 교보문고 · 쿠팡** 3개 구매 버튼은 제휴 링크를 지원합니다.
링크는 코드에 박혀 있지 않고 **환경변수로 주입**되므로, 제휴 승인 후 `.env` 값만
바꿔 끼우면 전 서점 버튼이 한 번에 제휴 링크로 바뀝니다. (미설정 시 일반 검색 링크로 동작)

동작 방식(`lib/affiliate.ts`): 각 서점 템플릿에 검색어 자리표시자 `{query}` 를 넣으면
책 제목이 자동으로 채워집니다. 앱은 Claude 가 준 `yes24_query`(=책 제목)를 검색어로 씁니다.

### 쿠팡 파트너스 (이미 연결됨)

현재 파트너스 ID **`AF8666448`** 가 기본값으로 들어가 있어, 쿠팡 버튼은 아래 형식으로 열립니다.
...
두 서점은 보통 **제휴 네트워크**(예: 링크프라이스 등)나 자체 파트너 프로그램을 통해
링크를 발급받습니다. 승인 후 받은 **추적 링크 형식**을 템플릿으로 넣으세요.
링크 안의 목적지 URL(검색/상품 페이지) 자리에 `{query}` 를 끼워 넣으면 됩니다.

```
# 예시(형식은 발급받은 실제 값으로 대체하세요)
NEXT_PUBLIC_YES24_LINK_TEMPLATE=https://<제휴추적도메인>/redirect?pid=<내ID>&url=https%3A%2F%2Fwww.yes24.com%2FProduct%2FSearch%3Fquery%3D{query}
NEXT_PUBLIC_KYOBO_LINK_TEMPLATE=https://<제휴추적도메인>/redirect?pid=<내ID>&url=https%3A%2F%2Fsearch.kyobobook.co.kr%2Fsearch%3Fkeyword%3D{query}
```

> 팁: 제휴 네트워크가 "목적지 URL을 통째로 인코딩해서 파라미터로 넘겨라"고 하면,
> `{query}` 는 그 인코딩된 URL **안쪽의 검색어 위치**에 넣으면 됩니다. (앱이 검색어만 인코딩)

### 정리 — "링크를 어떻게 넣죠?"의 답

- 쿠팡은 **이미 연결**되어 있어요(ID `AF8666448`). 바로 수수료 추적이 됩니다.
- 예스24·교보문고는 링크를 화면 코드에 붙이는 게 아니라 **환경변수만** 채우면 됩니다.
  `NEXT_PUBLIC_YES24_LINK_TEMPLATE`, `NEXT_PUBLIC_KYOBO_LINK_TEMPLATE`.
- 로컬은 `.env.local`, 배포는 Vercel **Settings → Environment Variables** 에 동일하게 추가 후 재배포.
- ⚠️ 제휴사 정책상 링크 형식은 바뀔 수 있으니, 위 예시는 **틀 참고용**이고 실제 값은
  각 파트너스/네트워크 대시보드에서 발급받은 그대로 사용하세요.
